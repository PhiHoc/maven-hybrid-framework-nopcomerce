...
public class Wishlist_Compare_RecentViewTest extends BaseTest {
    ...
    public void WishlistCompareRecentView_04_Add_Product_To_Compare() {
        log.info("Recently view - Step 01: Click to compare button of product: '"+ noteBookNames.get(0)+"'");
        homePO.clickToCompareButtonByProductName(noteBookNames.get(0));

        log.info("Recently view - Step 02: Verify add product to compare list success");
        Assert.assertEquals(homePO.getBarNotificationMessage(),"The product has been added to your product comparison");

        log.info("Recently view - Step 03: Click to Close message");
        homePO.clickToCloseNotificationMessageButton();

        log.info("Recently view - Step 04: Click to compare button of product: '"+ pcName +"'");
        homePO.clickToCompareButtonByProductName(pcName);

        log.info("Recently view - Step 05: Verify add product to compare list success");
        Assert.assertEquals(homePO.getBarNotificationMessage(),"The product has been added to your product comparison");

        log.info("Recently view - Step 06: Click to Close message");
        homePO.clickToCloseNotificationMessageButton();

        log.info("Recently view - Step 07: Click to 'Compare product' link");
        compareProductPO = (CompareProductPO) homePO.clickToFooterLinkByName("Compare products list");

        log.info("Recently view - Step 08: Verify first product name");
        Assert.assertEquals(compareProductPO.getCompareProductNameList().get(0), pcName);

        log.info("Recently view - Step 09: Verify second product name");
        Assert.assertEquals(compareProductPO.getCompareProductNameList().get(1), noteBookNames.get(0));

        log.info("Recently view - Step 10: Click to 'Clear Data' button");
        compareProductPO.clickToClearListButton();

        log.info("Recently view - Step 11: Verify no product message");
        Assert.assertEquals(compareProductPO.getNoCompareProductMessage(),"You have no items to compare.");

        log.info("Recently view - Step 12: Click home page logo to go back to homepage");
        homePO = wishListPO.clickToHomePageLogoLink();
    }
